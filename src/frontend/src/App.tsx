...
import { useSaveMatchLocal, useGetDefaultSettings } from './hooks/useQueries';
...
// Check if we're on the external display route
const isExternalRoute = window.location.pathname === '/external' || window.location.hash === '#/external';
...
function MainScoreboard() {
  ...
  const [winner, setWinner] = useState<{ side: 'Ao' | 'Aka'; name: string } | null>(null);
  ...
  const doSaveMatch = useCallback((winnerName: string, winnerSide: 'Ao' | 'Aka') => {
    matchCounter += 1;
  ...
  useEffect(() => {
    ...
    let detectedWinner: { side: 'Ao' | 'Aka'; name: string } | null = null;
  ...
  }, [timer.timeMs, timer.isRunning, ao.score, aka.score, ao.senshu, aka.senshu, ao.ippon, ao.wazaari, aka.ippon, aka.wazaari, ao.name, aka.name, doSaveMatch]);
  ...
  }, [ao, aka, timer.timerDisplay, timer.timeMs, timer.isRunning, tatamiNo, winner, mirrorExternal, category, darkMode]);

  // Determine the winner for a match being reset (tie-break logic)
  const resolveWinnerForSave = useCallback((): { side: 'Ao' | 'Aka'; name: string } => {
    if (ao.score > aka.score) {
  ...
  const handleSetMatchTime = useCallback(() => {
  ...
  const handleFileSettingsApplied = useCallback((mins: number, secs: number, tatami: string) => {
    setMatchTimeMinutes(mins);
    setMatchTimeSeconds(secs);
    setTatamiNo(tatami);
  ...
  const handleOpenExternalSB = useCallback(() => {
    const url = window.location.origin + window.location.pathname + '#/external';
    window.open(url, '_blank', 'width=1280,height=720');
  }, []);

  // Player state updaters
  const updateAo = useCallback((updater: (prev: PlayerPanelState) => PlayerPanelState) => {
    setAo(prev => updater(prev));
  }, []);

  const updateAka = useCallback((updater: (prev: PlayerPanelState) => PlayerPanelState) => {
  ...
  const handleAoToggleWarning = useCallback((idx: number) => {
    updateAo(prev => {
  ...
  const handleAkaToggleWarning = useCallback((idx: number) => {
    updateAka(prev => {
      const warnings = [...prev.warnings];
      const turningOn = !warnings[idx];
      warnings[idx] = turningOn;
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col leading-none">
            ...
            <span className={`text-[9px] italic ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>Software designed by Fenil Dav</span>
          </div>
        </div>
        <span className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          {category && <span className={`mr-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{category}</span>}
          Tatami: {tatamiNo}
        </span>
      </div>
      ...
      {/* Footer */}
      <footer className={`border-t border-gray-800 py-1 px-4 text-center shrink-0 ${darkMode ? 'bg-black' : 'bg-gray-100'}`}>
